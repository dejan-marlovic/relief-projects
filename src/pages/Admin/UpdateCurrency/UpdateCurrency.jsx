...
const validate = (values) => {
  const errors = {};
  if (!values.selectedId) errors.selectedId = "Please select a currency.";
  if (!values.name?.trim()) errors.name = "Currency name is required.";
  return errors;
};
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await authFetch(
        `${BASE_URL}/api/currencies/${Number(form.selectedId)}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        setFormError(
          data?.message ||
            data?.detail ||
            "There was a problem updating the currency.",
        );
        return;
      }
      ...
      setSuccessMessage(
        `Currency "${data?.name || form.name}" updated successfully.`,
      );
    } catch (err) {
      console.error("Update currency error:", err);
      setFormError(err?.message || "Unexpected error while updating currency.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className={styles.formContainer}>
        <div className={styles.pageHeader}>
          <div className={styles.pageHeaderText}>
            <h3 className={styles.pageTitle}>Update Currency</h3>
            <p className={styles.pageSubtitle}>
              Select an active currency and update its details.
            </p>
          </div>
        </div>
          ...
          <>
            <div className={styles.grid}>
              <div className={styles.card}>
                <div className={styles.cardHeader}>
                  ...
                  <div className={styles.cardMeta}>Active currencies only</div>
                </div>
              ...
              <div className={styles.card}>
                <div className={styles.cardHeader}>
                  ...
                  <div className={styles.cardMeta}>Name and description</div>
                </div>
            ...
            <div className={styles.bottomActions}>
              ...
              <button
                ...
                disabled={saving}
              >
                <FiSave /> {saving ? "Saving..." : "Update currency"}
              </button>
            </div>
          </>
        )}
