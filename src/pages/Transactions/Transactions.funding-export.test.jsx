...
test("export keeps exact funding and allocation values, current currency and no mixed grand totals or retired share columns", async () => {
  ...
  const transactions = [
    { id: 31, projectId: 2, budgetId: 8, appliedForAmount: "9999999999999999999.999", approvedAmount: "90.000", fundingCurrency: { source: "CURRENT_BUDGET_CONFIGURATION", availability: "AVAILABLE", currency: { id: 3, name: "SEK" } } },
    { id: 32, projectId: 2, budgetId: 9, appliedForAmount: null, approvedAmount: "12.125", fundingCurrency: { source: "CURRENT_BUDGET_CONFIGURATION", availability: "CURRENCY_UNAVAILABLE", currency: null } },
  ];
  ...
  expect(sheet.getRow(4).values.join(" ")).not.toMatch(/share/i);
  ...
  expect(first.getCell(8).numFmt).toBe("#,##0.000");
