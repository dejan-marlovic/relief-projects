...
const row = { id: 7, projectId: 1, employeeId: 2, documentName: "Report.pdf", category: "FINANCE", documentDate: "2024-02-29", uploadedByUsername: "original.user", uploadedByUserId: 4, uploadedAt: "2026-09-19T10:00:00Z" };
...
test("uses backend labels, preserves a date-only value and separates uploader from employee", async () => {
  ...
  expect(fetch.mock.calls.some(([url]) => url.endsWith("?category=UNCATEGORIZED"))).toBe(true);
...
test("a late unfiltered response cannot replace filtered results", async () => {
  ...
  fetch.mockImplementation((url, options) => url.endsWith("/project/1") ? new Promise((resolve) => { finish = resolve; }) : normalFetch(url, options));
  mount();
